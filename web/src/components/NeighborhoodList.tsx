...
type SortKey = 'name' | 'commuteMinutes' | 'medianRent'
...
function SafetyStars({ rating }: { rating: number }) {
  ...
  return (
    <span
      style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 12, color }}
      title={labels[rating]}
    >
...
function LegBreakdown({ legs, city }: { legs: LegSummary; city: 'nyc' | 'sf' }) {
  const isSF = city === 'sf'
  // Driving estimate overrides transit breakdown entirely
  if (legs.carMinutes) {
    return (
      <tr>
        <td colSpan={3} style={{ padding: '6px 12px 10px', background: '#f0f6ff', borderBottom: '1px solid #dde6f5' }}>
          <span
            style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: '#444' }}
            title="Transit unavailable or walk to nearest stop exceeds 20 min — driving estimate shown instead."
          >
            <span style={{ fontSize: 16 }}>🚗</span>
            <span style={{ fontWeight: 600 }}>{legs.carMinutes} min</span>
            <span style={{ color: '#888' }}>Drive est. · no practical transit</span>
          </span>
        </td>
      </tr>
    )
...
function NeighborhoodDetail({ n }: { n: Neighborhood }) {
  const hasTemps =
    n.summerHigh != null && n.summerLow != null && n.winterHigh != null && n.winterLow != null
  if (n.safetyRating == null && !n.description && !hasTemps) return null
  return (
    <tr>
      <td
        colSpan={3}
      ...
      >
        ...
        {hasTemps && (
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: n.description ? 6 : 0 }}>
...
export default function NeighborhoodList({ neighborhoods, selectedNta, onSelect, city = 'nyc' }: Props) {
  ...
  return (
    <table className="sidebar-table">
      <thead>
        <tr>
          <th onClick={() => handleSort('name')}>Neighborhood{arrow('name')}</th>
          <th onClick={() => handleSort('commuteMinutes')}>Commute{arrow('commuteMinutes')}</th>
          <th onClick={() => handleSort('medianRent')}>Rent{arrow('medianRent')}</th>
        </tr>
      </thead>
      <tbody>
        {sorted.map((n) => (
